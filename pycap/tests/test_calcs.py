...
def theis_results():
    ...
    Q = [float(i)*pycap.GPM2CFD for i in [p.loc['Pumping Rate Well #1 (gpm)'],
...
def test_theis(theis_results):
    ...
    time = pars['time']
    dd = [pycap.theis(pars['T'], pars['S'], time, dist, currQ) for currQ in pars['Q']]
...
def test_walton(walton_results):
    ...
    rch={}
    for idx in [0,1]:
        dep[idx] = pycap.walton(pars['T_gpd_ft'][idx],
                    pars['S'][idx],
                    res.t_well,
                    pars['dist'][idx],
                    pars['Q'][idx]
                    )
        rch[idx] = pycap.walton(pars['T_gpd_ft'][idx],
                    pars['S'][idx],
                    res.t_image,
                    pars['dist'][idx],
                    pars['Q'][idx]
...
def test_geoprocessing(SIR2009_5003_Table2_Batch_results):
    ...
    nearest['analytical_removal'] = nearest['distance'].apply(lambda dist: pycap.hunt99(T, S, time, dist, Q, streambed=streambed)* 448.83116885)
...
def test_ward_lough_depletion(ward_lough_test_data):
    ...
    dQ2_test['mod']  = pycap.WardLoughDepletion(T1,T2,S1,S2,width,Q,dist,streambed_thick,
...
def test_ward_lough_drawdown(ward_lough_test_data):
    ...
    s1_test['mod'], _  = pycap.WardLoughDrawdown(T1,T2,S1,S2,width,Q,dist,streambed_thick,
                                streambed_K,aquitard_thick,aquitard_K,s1_test.index*100, x, y)
    _, s2_test['mod']  = pycap.WardLoughDrawdown(T1,T2,S1,S2,width,Q,dist,streambed_thick,
